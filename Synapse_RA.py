...
for i in range(0, len(x0)):
    ...
    W = topology_dis()
    xd = odeint(dx1, x0, t, args=(np.where(I11 > 0.05, I11, 0),W,))

fig = plt.figure(figsize=(6, 6))
plt.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=0.2, hspace=0.5)
plt.plot(xx, GAUSS1, 'royalblue',linewidth=3,alpha=0.2, label='I$^{ext}$');
plt.plot(xx, GAUSS2, 'royalblue',linewidth=3,alpha=0.2);
plt.plot(xx, np.maximum(np.array(np.float32(xd[700, 0: 128])), 0), 'Crimson',linewidth=3,marker="*",markersize=8, label='Model');
plt.xlabel('Neurons Label(radian)', fontsize=18);
plt.ylabel('u', fontsize=18);
plt.ylim(-0.03, 0.25)
plt.xlabel('Neurons Label x (radian)', fontsize=18);
plt.ylabel('Synaptic Current u', fontsize=18);
plt.show()
print()